...
.right-column[
Let's skip syntax for now and have a look at the following code for a virtual store
which will allow a customer to pay using some `PaymentInfo`:

```scala
class Store {
  def buyItem(info: PaymentInfo,
      payments: PaymentService): Item = {
    val item = new Item()
...
- **This code is not reusable**: if we want to buy multiple items we either call this
  multiple time or create a new function that is specific for batching payments
...
state is immutable it can be safely shared between threads without the need for locking.
